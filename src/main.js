...
ace.define("ace/mode/svelte_completions", ["require", "exports", "module", "ace/token_iterator"], function(require, exports, module) {
  "use strict";
  var TokenIterator = require("../token_iterator").TokenIterator;
  var commonAttributes = [
    "accesskey",
  ...
  var SvelteCompletions = function() {
  };
  (function() {
    this.getCompletions = function(state, session, pos, prefix) {
      var token = session.getTokenAt(pos.row, pos.column);
  ...
  }).call(SvelteCompletions.prototype);
  exports.SvelteCompletions = SvelteCompletions;

});

ace.define("ace/mode/folding/svelte", ["require", "exports", "module", "ace/lib/oop", "ace/mode/folding/mixed", "ace/mode/folding/html", "ace/mode/folding/cstyle"], function(require, exports, module) {
  "use strict";
  const oop = require("../../lib/oop");
  const MixedFoldMode = require("./mixed").FoldMode;
  const HtmlFoldMode = require("./html").FoldMode;
  const CStyleFoldMode = require("./cstyle").FoldMode;
  const FoldMode = exports.FoldMode = function(voidElements, optionalTags) {
    MixedFoldMode.call(this, new HtmlFoldMode(voidElements, optionalTags), {
      "js-": new CStyleFoldMode(),
      "css-": new CStyleFoldMode(),
      "scss-": new CStyleFoldMode(),
      "less-": new CStyleFoldMode(),
      "ts-": new CStyleFoldMode(),
    });
  };
  oop.inherits(FoldMode, MixedFoldMode);
});

ace.define("ace/mode/svelte", ["require", "exports", "module"], function (require, exports, module) {
  "use strict";
  
  const oop = require("ace/lib/oop");
  const lang = require("../lib/lang");
  const HtmlMode = require("ace/mode/html").Mode;
  const ScssMode = require("ace/mode/scss").Mode;
  const CssMode = require("ace/mode/css").Mode;
  const LessMode = require("ace/mode/less").Mode;
  const TsMode = require("ace/mode/typescript").Mode;
  ...
  const ScssHighlightRules = require("ace/mode/scss_highlight_rules").ScssHighlightRules;
  const LessHighlightRules = require("ace/mode/less_highlight_rules").LessHighlightRules;
  const TypescriptHighlightRules = require("ace/mode/typescript_highlight_rules").TypeScriptHighlightRules;
  const JavascriptHighlightRules = require("ace/mode/javascript_highlight_rules").JavaScriptHighlightRules;
  const SvelteFoldMode = require("ace/mode/folding/svelte").FoldMode;
  const SvelteCompletions = require("ace/mode/svelte_completions").SvelteCompletions;
  
  const SvelteHighlightRules = function () {
    HtmlHighlightRules.call(this)
    
    const svelte = {
      token: "paren.lparen.level3.svelte",
      regex: /{/,
    ...
    for (const key in this.$rules) {
      ...
      this.$rules[key].unshift(svelte);
    }
    
    this.embedRules(JavascriptHighlightRules, "inline-js-", [{
      regex: /}/,
      token: "paren.rparen.level3.svelte",
      ...
      token: "keyword",
      regex: "(?<=[#:\\/])(?:if|else(?:\\s+if)?|each|await|then|catch|key)",
    },{
      token: "variable.parameter",
      regex: /@(?:html|debug|const)/,
    }]);
    
    const overwriteJSXendRule = function (prefix) {
    ...
    const embedLangRules = (HighlightRules, prefix, tag) => {
      this.$rules.attributes.unshift({
        regex: "(?=lang\\s*=\\s*" + "['\"]" + prefix.slice(0, -1) + "['\"])",
        push: prefix + "lang"
      });
      
      this.$rules[prefix + "lang"] = [{
        token: ["entity.other.attribute-name.xml", "text", "keyword.operator.attribute-equals.xml", "text", "support.constant"],
        regex: "\\b(lang)(\\s*)(=)(\\s*)(" + "['\"]" + prefix.slice(0, -1) + "['\"])",
        push: [
          { include: "attributes" },
          {
            token: "meta.tag.punctuation.tag-close.xml",
    ...
    embedLangRules(ScssHighlightRules, "scss-", "style")
    embedLangRules(LessHighlightRules, "less-", "style")
    embedLangRules(TypescriptHighlightRules, "ts-", "script")
    
    this.normalizeRules();
  };
  
  oop.inherits(SvelteHighlightRules, HtmlHighlightRules);
  ...
  const SvelteMode = function () {
    HtmlMode.call(this)
    this.HighlightRules = SvelteHighlightRules;
    this.createModeDelegates({
      "css-": CssMode,
      "js-": JsMode,
      "scss-": ScssMode,
    ...
    this.$completer = new SvelteCompletions();
    this.foldingRules = new SvelteFoldMode(this.voidElements, lang.arrayToMap(optionalEndTags));
  };
  ...
  !function () {
    ...
    this.$id = "ace/mode/svelte"; 
  }.call(SvelteMode.prototype)
  
  oop.inherits(SvelteMode, HtmlMode);
  exports.Mode = SvelteMode;
});
...
function main() {
  if (+BuildInfo.versionCode >= 292) {
    acode.require('aceModes').addMode('svelte', 'svelte', 'Svelte');
  } else {
    ace.require("ace/ext/modelist").modes.push({
      caption: "Svelte",
      extRe: /\.svelte$/,
      extensions: "svelte",
      mode: "ace/mode/svelte",
      name: "svelte"
    })
  }
}
...
function destroy() {
  if (+BuildInfo.versionCode >= 292) {
    acode.require('aceModes').removeMode("svelte")
  } else {
